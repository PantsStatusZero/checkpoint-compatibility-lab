...
table=pq.ParquetFile(io.BytesIO(p)).read(columns=["precursor_mz","smiles","adduct"])
out=io.BytesIO()
with gzip.GzipFile(fileobj=out,mode="wb",compresslevel=9,mtime=0) as gz:
    ...
    w.writerow(["precursor_mz","smiles","adduct"])
    cols=[table.column(i).to_pylist() for i in range(3)]
...
Path("results/raw-index-minimal-csv.cipher").write_bytes(cipher)
manifest={
 "state":"PASS","format":"csv.gz","rows":table.num_rows,"columns":table.column_names,
 "source_zip_sha256":EXPECTED_ZIP,"source_parquet_sha256":EXPECTED_PARQUET,
 "plaintext_sha256":sha(plain),"plaintext_bytes":len(plain),
 ...
 "nonce_b64":base64.b64encode(nonce).decode(),"tag_b64":base64.b64encode(enc.tag).decode(),
 "transport":"RSA-OAEP-SHA256 + AES-256-GCM","plaintext_payload_uploaded":False
}
Path("results/raw-index-minimal-csv-manifest.json").write_text(json.dumps(manifest,indent=2,sort_keys=True),encoding="utf-8")
